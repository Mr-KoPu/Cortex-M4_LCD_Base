	...
	if (ucGPIX == 1)	/* 	GPIX = 1 表示 4.3 寸屏 480x272 */
	{
		...
		RA8875_WriteData(2);		/* PLLDIVK[2:0] = 2, 除以4 */
            ...
            /* RA8875 的内部系统频率 (SYS_CLK) 是结合振荡电路及PLL 电路所产生，频率计算公式如下 :
              SYS_CLK = FIN * ( PLLDIVN [4:0] +1 ) / ((PLLDIVM+1 ) * ( 2^PLLDIVK [2:0] )) = 25M * (10 + 1) / ((0 + 1) * (2 ^ 2)) = 68.75MHz
            */
	...
	*/
	...
	/* 05H  REG[05h] Serial Flash/ROM Configuration Register (SROC)
		7	Serial Flash/ROM I/F # 选择
				0:选择Serial Flash/ROM 0 接口。[RA8875驱动板图库芯片接在 1 接口]
...
*	功能说明: 在LCD上显示一个BMP位图，位图点阵扫描次序:从左到右，从上到下 水平扫描
*	形    参:
*			_usX, _usY : 图片的坐标
*			_usHeight  :图片高度
*			_usWidth   :图片宽度
...
{
	/*
		RA8875 通过使用 — 上海集通公司 (Genitop Inc) 外部串行式字体内存 (Font ROM)，可支持各样
		...
		此屏RA8875驱动板集成的字库芯片为 集通字库芯片_GT23l32S4W
