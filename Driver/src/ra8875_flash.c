		...
		if (ucTouch != TOUCH_NONE)
		{
			switch (ucTouch)
			{
				case TOUCH_DOWN:		/* 触笔按下事件 */
					...
					{
						FormW25->BtnRet.Focus = 1;
						LCD_DrawButton(&FormW25->BtnRet);
					}
					else if (TOUCH_InRect(tpX, tpY, BTN1_X, BTN1_Y, BTN1_H, BTN1_W))
					{
						FormW25->Btn1.Focus = 1;
						LCD_DrawButton(&FormW25->Btn1);
					}
					else if (TOUCH_InRect(tpX, tpY, BTN2_X, BTN2_Y, BTN2_H, BTN2_W))
					{
						FormW25->Btn2.Focus = 1;
						LCD_DrawButton(&FormW25->Btn2);
					}
					else if (TOUCH_InRect(tpX, tpY, BTN3_X, BTN3_Y, BTN3_H, BTN3_W))
					{
						FormW25->Btn3.Focus = 1;
						LCD_DrawButton(&FormW25->Btn3);
					}
					else if (TOUCH_InRect(tpX, tpY, BTN4_X, BTN4_Y, BTN4_H, BTN4_W))
					{
						FormW25->Btn4.Focus = 1;
						LCD_DrawButton(&FormW25->Btn4);
					}
					else if (TOUCH_InRect(tpX, tpY, BTN5_X, BTN5_Y, BTN5_H, BTN5_W))
					{
						FormW25->Btn5.Focus = 1;
						LCD_DrawButton(&FormW25->Btn5);
					}
					else if (TOUCH_InRect(tpX, tpY, BTN6_X, BTN6_Y, BTN6_H, BTN6_W))
					{
						FormW25->Btn6.Focus = 1;
						LCD_DrawButton(&FormW25->Btn6);
					}	
					else if (TOUCH_InRect(tpX, tpY, BTN7_X, BTN7_Y, BTN7_H, BTN7_W))
					{
						FormW25->Btn7.Focus = 1;
...
void ReadRA8875Flash(uint8_t _ucChipType)
{
	...
	{
		/* 挂载文件系统 */
		result = f_mount(ucVolume, &fs);			/* Mount a logical drive */
		if (result != FR_OK)
		{
                  printf("挂载文件系统失败 (%d)\r\n", result);
...
void WriteRA8875Flash(uint8_t _ucChipType)
{
	...
	for (i = 0; i < WriteSize / 512; i++)
	{
		...
		{
			w25_EraseSector(i * 512);
		}
		w25_WritePage(buf, i * 512, 512);		/* 写入串行Flash */
		w25_ReadBuffer(buf_read, i * 512, 512);	/* 读回进行比较 */
